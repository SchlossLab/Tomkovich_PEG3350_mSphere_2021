...
five_d_stools_variables <- tibble(unique_label = attr(five_d_stools, "Labels")) %>%
  left_join(metadata, by = "unique_label")
five_d_stools_adonis <- adonis(five_d_stools~(group/(exp_num*unique_cage_no*ext_plate*miseq_run))*day, strata = five_d_stools_variables$unique_mouse_id, data = five_d_stools_variables, permutations = 1000, parallel = 10)
