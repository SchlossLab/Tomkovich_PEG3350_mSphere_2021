...
#Test for OTU differences between cleared and colonized mice----
#Use same timepoint (5dpi) used as input data for predictive model
clearvcol_otus <- agg_otu_data %>% 
  filter(day == 5) %>% 
  filter(!clearance_status_d10 == "no_data") %>% #Remove mice that did not have clearance_status_d10
  group_by(otu) %>% 
  nest() %>% 
  mutate(model=map(data, ~wilcox.test(.x$agg_rel_abund ~ .x$clearance_status_d10, paired = FALSE) %>% tidy())) %>% 
  mutate(median = map(data, get_rel_abund_median_clearance_status)) %>% 
  unnest(c(model, median)) %>% 
  ungroup() 
#Adjust p-values for testing multiple genera
clearvcol_otus_adjust <- clearvcol_otus  %>%
  select(otu, statistic, p.value, method, alternative, cleared, colonized) %>% 
  mutate(p.value.adj=p.adjust(p.value, method="BH")) %>% 
  arrange(p.value.adj)

#Examine significant OTUs
sig_clearvcol_otus <- clearvcol_otus_adjust %>% 
  filter(p.value.adj < 0.05)

#Significant Porphyromonadaceae OTUs
sig_porp_otus <- sig_clearvcol_otus %>% 
  filter(str_detect(otu, "Porphyromonadaceae")) 
#8 Porphyromodaceae OTU

#Remove any OTUs that were already plotted because they are a close match to Muribaculum
sig_porp_otus <- sig_porp_otus %>% 
  filter(!otu %in% muribac_otus)%>% 
  pull(otu)
#Removes 2 OTUs

#Significant Akkermansia OTUs
sig_akkermansia_otus <- sig_clearvcol_otus %>% 
  filter(str_detect(otu, "Akkermansia"))
#1 OTU Akkermansia OTU 3

#Significant Enterobacteriaceae OTUs
sig_entero_otus <- sig_clearvcol_otus %>% 
  filter(str_detect(otu, "Enterobacteriaceae"))
#1 OTU Enterobacteriaceae OTU 2

#Significant Lachnospiraceae OTUs
sig_lachno_otus <- sig_clearvcol_otus %>% 
  filter(str_detect(otu, "Lachnospiraceae")) %>% 
  pull(otu)
#29 Lachnospiraceae OTUs

#Significant Bacteroides OTUs
sig_bacteroides_otus <- sig_clearvcol_otus %>% 
  filter(str_detect(otu, "Bacteroides"))
#1 OTU Bacteroides OTU 1

#Examine other Porphyromondaceae OTUs that were significantly diff. between cleared and colonized mice at 5dpi----
porphyr_otus <- sig_porp_otus[1:4] #Select only top 4 OTUs (in addition to 6 & 8 that were plotted as Muribaculum OTUs)
porphyr_otu_names <- agg_otu_data %>% filter(otu %in% porphyr_otus) %>% 
  filter(!duplicated(otu_name)) %>% 
  pull(otu_name)
#Line plots instead of area plot to convey how the most abundant genera that were top contributors to machine learning models were changing over time
...
lachno_otus <- sig_lachno_otus[1:4]
lachno_otu_names <- agg_otu_data %>% filter(otu %in% lachno_otus) %>% 
  filter(!duplicated(otu_name)) %>% 
  pull(otu_name)
#Line plots instead of area plot to convey how the most abundant genera that were top contributors to machine learning models were changing over time
...
#Alternative lineplot version with symbols for individual mice in addition to the median line
...
porphyr_otus_df <- agg_otu_data %>% 
  ...
  filter(otu %in% porphyr_otus) %>%
  #Reorder genera to match contribution to model
  mutate(otu_name = fct_relevel(otu_name, porphyr_otu_names)) 
porphyr_otus_median <- porphyr_otus_df %>% #Reorganize otu_names to match OTU number
  group_by(clearance_status_d10, otu_name, day) %>% 
  summarize(median=median(agg_rel_abund + 1/2000),`.groups` = "drop")  #Add small value (1/2Xsubssampling parameter) so that there are no infinite values with log transformation
porphyr_otus_line_plot <-   ggplot(NULL)+
  geom_point(porphyr_otus_df, mapping = aes(x=day, y=agg_rel_abund, color=clearance_status_d10), size  = 1.5, alpha = 0.6, position = position_dodge(width = 0.6))+
  geom_line(porphyr_otus_median, mapping = aes(x = day, y=median, color=clearance_status_d10, group = clearance_status_d10), linetype = "solid", alpha = 0.6)+
  scale_colour_manual(name=NULL,
                      values=c("blue", "red"),
                      breaks=c("cleared", "colonized"),
                      labels=c("cleared", "colonized"))+
